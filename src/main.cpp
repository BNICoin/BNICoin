...
#if defined(NDEBUG)
# error "BNI cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "BNI Signed Message:\n";
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!(IsCoinBase() || IsCoinStake()))
        return 0;
    return max(0, (nCoinbaseMaturity+42) - GetDepthInMainChain());
}
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if (nActualSpacing < 0 && nBestHeight > 0)
    ...
    if (pindexBest->nHeight > 0 && pindexBest->nHeight < 999999)
