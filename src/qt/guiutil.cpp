...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("BNI://"))
    {
        uri.replace(0, 12, "BNI:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BNI.lnk";
}
...
{
    return GetAutostartDir() / "BNI.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BNI-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  BNI-qt [" + tr("command-line options") + "]                     " + "\n";
